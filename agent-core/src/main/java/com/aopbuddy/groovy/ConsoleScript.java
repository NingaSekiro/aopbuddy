...
public abstract class ConsoleScript extends Script {
  ...
  public void hotswap(String className, byte[] classBuffer) throws UnmodifiableClassException {
    ...
    inst.retransformClasses(Arrays.stream(allLoadedClasses)
        .filter(clz -> clz.getName().equals(className)
        ).toArray(Class[]::new));
  }
