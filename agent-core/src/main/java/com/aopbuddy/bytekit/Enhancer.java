...
import static com.aopbuddy.retransform.Context.isIgnore;
import static com.aopbuddy.retransform.Context.key;
...
public class Enhancer implements ClassFileTransformer {
  ...
  public byte[] transform(ClassLoader inClassLoader, String className, Class<?> classBeingRedefined,
    ...
    try {
      ...
      List<Advisor> filteredAdvisors = Context.ADVISORS.stream()
          .filter(advisor -> advisor.getPointcut().matchesClassName(normalizedClassName)).
      ...
      interceptorProcessors.addAll(
          defaultInterceptorClassParser.parse(SpyInterceptors.SpyInterceptor1.class));
      interceptorProcessors.addAll(
          defaultInterceptorClassParser.parse(SpyInterceptors.SpyInterceptor2.class));
      interceptorProcessors.addAll(
          defaultInterceptorClassParser.parse(SpyInterceptors.SpyInterceptor3.class));
      ...
      LocationFilter enterFilter = new InvokeContainLocationFilter(
          Type.getInternalName(SpyAPI.class), "atEnter",
          LocationType.ENTER);
      LocationFilter existFilter = new InvokeContainLocationFilter(
          Type.getInternalName(SpyAPI.class), "atExit",
          LocationType.EXIT);
      ...
      for (MethodNode methodNode : classNode.methods) {
        if (!isIgnore(methodNode)) {
          for (Advisor filteredAdvisor : filteredAdvisors) {
            if (!filteredAdvisor.getPointcut().matchesMethodName(methodNode.name)) {
              continue;
            }
