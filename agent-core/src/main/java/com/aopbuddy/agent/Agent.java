...
public class Agent {
  ...
  private static final String SPY_API_JAR = "spy-api-1.0.jar";
  ...
  private static void initSpy(Instrumentation instrumentation) throws Throwable {
    ...
    if (spyClass == null) {
      ...
      if (codeSource != null) {
        ...
        File spyJarFile = new File(arthasCoreJarFile.getParentFile(), SPY_API_JAR);
        instrumentation.appendToBootstrapClassLoaderSearch(new JarFile(spyJarFile));
      } else {
        throw new IllegalStateException("can not find " + SPY_API_JAR);
      }
    }
  }
}
